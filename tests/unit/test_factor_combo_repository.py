...
from db.factor_combo_repository import FactorComboRepository, RegisteredFactorChoice
...
class StubPreparationTransaction:
    ...
    def __init__(
        ...
        *,
        ...
        参数 ``execution_results`` 可为连续写操作提供不同的 ``lastrowid``；未提供时复用 ``execution_result``。
        """
...
class StubPreparationDatabaseClient:
    ...
    def __init__(
        ...
        *,
        ...
        fetch_all_responses: list[list[dict[str, Any]]] | None = None,
    ) -> None:
        """保存 Repository 直连查询、多行查询和事务替身。"""

        self.fetch_one_responses = list(fetch_one_responses)
        self.fetch_all_responses = list(fetch_all_responses or [])
        self.transaction_stub = transaction
        self.queries: list[tuple[str, tuple[Any, ...] | None]] = []
    ...
    def fetch_all(
        ...
        """记录 Repository 多行查询并返回下一组预置结果。"""

        self.queries.append((query, parameters))
        if not self.fetch_all_responses:
            raise AssertionError("no preparation client fetch_all response remains")
        return self.fetch_all_responses.pop(0)
...
class TestFactorComboRefreshEvidenceRepository:
    ...
    def test_cleanup_version_query_includes_feedback_next_versions(self) -> None:
        """清理版本图必须包含表单当前指针及 feedback 的来源和下一版本。"""

        transaction = StubCleanupTransaction([[{"id": 701}, {"id": 702}]])
        repository = FactorComboRepository(StubCleanupDatabaseClient(transaction), "test")  # type: ignore[arg-type]

        rows = repository._fetch_versions_for_forms(transaction, [41])

        assert rows == [{"id": 701}, {"id": 702}], rows
        query, parameters = transaction.queries[0]
        assert "factor_combo_form AS form" in query, query
        assert "source_factor_combo_version_id" in query, query
        assert "next_factor_combo_version_id" in query, query
        assert parameters == (41, 41, 41, 41), parameters

    def test_registration_cleanup_query_uses_version_hash_as_identity(self) -> None:
        """登记清理不能因无关业务 combo_id 与当前版本主键数字相同而误判共享引用。"""

        transaction = StubCleanupTransaction([[]])
        repository = FactorComboRepository(StubCleanupDatabaseClient(transaction), "test")  # type: ignore[arg-type]

        rows = repository._fetch_registrations(transaction, [701], ["hash-701"])

        assert rows == [], rows
        query, parameters = transaction.queries[0]
        assert "registered.combo_version_hash IN" in query, query
        assert "registered.combo_id IN" not in query, query
        assert parameters == (701, "hash-701"), parameters
...
class TestFactorComboRegistrationRepository:
    ...
    def test_registered_refresh_candidate_requires_core_metrics_slices_and_sources(self) -> None:
        """真实登记样本查询应同时限定核心汇总、原始切片、有效性、组件和来源关系。"""
            ...
            [[
                {
                    ...
                    "parent_factor_id": 71,
                    "version_id": 701,
                    "factor_detail_id": 901,
                    "registration_validity_status_id": 1001,
                    "sub_factor_name": "combo-factor-a",
                    "combo_version_hash": version_hash,
                }
            ]]
        )
        ...
        choice = repository.find_registered_factor_with_refresh_evidence()

        assert choice == RegisteredFactorChoice(
            registration_id=501,
            sub_factor_id=801,
            parent_factor_id=71,
            version_id=701,
        ...
        query, parameters = client.queries[0]
        assert parameters is None, parameters
        assert "summary.ic_scope = 'time_series'" in query, query
        assert "summary.ic_scope = 'cross_sectional'" in query, query
        assert "summary.monotonicity_ratio IS NOT NULL" in query, query
        assert "factor_ic_slice_metrics" in query, query
        assert "factor_combo_component" in query, query
        assert "factor_sub_factor_relations" in query, query
        assert "sub_factor_parent_relations" in query, query
...
class TestFactorComboTestDataPreparationRepository:
    ...
    def test_multiple_parent_preparation_creates_and_reads_back_test_relations(self) -> None:
        """可展开母因子不足时创建临时母因子，并把其关系纳入当前会话归属。"""

        one_parent_rows = [
            {
                "factor_id": 11,
                "factor_name": "parent-a",
                "sub_factor_id": 1001,
                "sub_factor_name": "sub-a",
            },
            {
                "factor_id": 11,
                "factor_name": "parent-a",
                "sub_factor_id": 1002,
                "sub_factor_name": "sub-b",
            },
        ]
        source_sub_factor_rows = [
            {
                "parent_factor_id": 11,
                "parent_factor_name": "parent-a",
                "sub_factor_id": 1001,
                "sub_factor_name": "sub-a",
            },
            {
        ...
        final_rows = [
            *one_parent_rows,
            {
                "factor_id": 701,
                "factor_name": "__questtest_unrelated_parent__created",
                "sub_factor_id": 1001,
                "sub_factor_name": "sub-a",
            },
            {
            ...
            {
                "factor_id": 702,
                "factor_name": "__questtest_unrelated_parent__created-2",
                "sub_factor_id": 1001,
                "sub_factor_name": "sub-a",
            },
            {
        ...
        source = {
            ...
            "factor_name": "parent-a",
            "cn_name": "parent-a-cn",
            "serial_number": "parent-a-serial",
            "serial_prefix": "factor",
            "level": 1,
            "child_factor_count": 2,
            "metadata": {"source": True},
        }
        schema = [{"Field": column, "Extra": ""} for column in source]
        transaction = StubPreparationTransaction(
            fetch_one_responses=[source, source],
            fetch_all_responses=[schema, schema],
            execution_results=[
                ExecutionResult(rowcount=1, lastrowid=701),
                ExecutionResult(rowcount=1, lastrowid=None),
                ExecutionResult(rowcount=1, lastrowid=None),
                ExecutionResult(rowcount=1, lastrowid=702),
                ExecutionResult(rowcount=1, lastrowid=None),
                ExecutionResult(rowcount=1, lastrowid=None),
            ],
        )
        client = StubPreparationDatabaseClient(
            fetch_one_responses=[],
            fetch_all_responses=[one_parent_rows, source_sub_factor_rows, final_rows],
            transaction=transaction,
        )
        repository = FactorComboRepository(client, "test")  # type: ignore[arg-type]

        parents = repository.ensure_parent_choices_for_test(41)

        assert len(parents) == 2, parents
        assert {parent.factor_id for parent in parents} == {11, 701}, parents
        assert repository._test_parent_factor_ids_by_form == {41: {701, 702}}
        assert repository._test_parent_relation_pairs_by_owner == {
            41: {(701, 1001), (701, 1002), (702, 1001), (702, 1002)}
        }
        assert sum("INSERT INTO factor_sub_factor_relations" in query for query, _ in transaction.executions) == 4

    def test_session_owned_parent_cleanup_deletes_owned_relations_before_factor(self) -> None:
        """会话归属的临时母因子清理时先删除测试关系，再删除母因子。"""

        transaction = StubPreparationTransaction(
            fetch_one_responses=[
                {"id": 701, "factor_name": "__questtest_unrelated_parent__created"},
                None,
            ],
            fetch_all_responses=[],
        )
        ...
        repository._test_parent_factor_ids_by_form[41] = {701}
        repository._test_parent_relation_pairs_by_owner[41] = {(701, 1001), (701, 1002)}

        cleaned = repository._clean_test_parent_factors(transaction, [41])

        assert cleaned == {701}
        executed_sql = [query for query, _ in transaction.executions]
        relation_indexes = [
            index for index, query in enumerate(executed_sql)
            if "DELETE FROM factor_sub_factor_relations" in query
        ]
        factor_index = next(index for index, query in enumerate(executed_sql) if "DELETE FROM factors" in query)
        assert relation_indexes and max(relation_indexes) < factor_index, executed_sql
