...
class TestFactorScenario:
    ...
    def test_fs_01_theme_lifecycle_create_list_detail_update_status(self, factor_resource_api, test_data_factory, resource_tracker):
        ...
        if status_response.status_code != 200:
            errors.append(f"status_code={status_response.status_code}")
        errors.extend(JSONResponseAssertionService.success_errors(status_response_body))
        if errors:
    ...
    @allure.title("FS-03 子因子创建-列表-详情-更新-状态链路")
    def test_fs_03_sub_factor_lifecycle_create_list_detail_update_status(self, factor_resource_api, test_data_factory, resource_tracker):
        """Case ID: FS-03
        测试目的: 验证子因子创建后可以查询、更新和变更状态。

        请求参数:
            使用 auto sub_factor_name 创建子因子并登记清理，随后查询列表、详情、更新 cn_name 和置状态 3。
        返回值:
            链路内核心接口应返回成功响应，最终子因子详情状态应为 3。
        """
