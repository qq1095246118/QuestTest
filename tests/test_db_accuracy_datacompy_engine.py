...
def test_datacompy_engine_fails_joined_row_with_source_only_payload_column(tmp_path):
    db_frame = pl.DataFrame(
        {
            "symbol": ["BTCUSDT"],
            "interval": ["1m"],
    ...
    assert payload["source_only_count"] == 0
    assert payload["unequal_count"] == 1
    assert payload["unequal_sample"][0]["columns"] == ["close"]
    assert payload["unequal_sample"][0]["db"] == {"close": None}
    assert payload["unequal_sample"][0]["source"] == {"close": "2"}


def test_datacompy_engine_handles_payload_columns_that_end_with_source(tmp_path):
    db_frame = pl.DataFrame(
        {
            "symbol": ["BTCUSDT"],
            "interval": ["1m"],
    ...
    payload = json.loads((tmp_path / result.diff_path).read_text(encoding="utf-8"))
    assert payload["unequal_count"] == 1
    assert payload["unequal_sample"][0]["columns"] == ["open__source"]


def test_datacompy_engine_safe_names_include_hash_for_sanitized_collisions(tmp_path):
    frame = pl.DataFrame(
        {
            "symbol": ["BTCUSDT"],
            "interval": ["1m"],
    ...
    underscore_result = engine.compare(
        shard_label="a_b",
        partition_label="1704067200000-1704153599999",
        db_frame=frame,
        source_frame=frame,
    ...
    assert slash_result.report_path != underscore_result.report_path
    assert slash_result.diff_path != underscore_result.diff_path
    assert (tmp_path / slash_result.report_path).exists()
    assert (tmp_path / underscore_result.report_path).exists()
