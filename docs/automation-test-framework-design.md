...
计划内的业务场景不得因为测试库当前没有恰好匹配的自然数据而使用 `pytest.skip`。Repository 应优先查询可复用的真实测试数据；
...
真实 Run 启动后暂时保护对应表单；即使 Worker/直接 API 流程没有经过 Service，Repository 发现表单仍带有活动或未知 `pipeline_run_id` 时也必须保留整组资源。未由当前 Scope 创建的会话/表单不得进入清理集合。所有 DB 写操作仅允许测试环境，生产环境不得配置写入凭据。
